...
test_that("results from a new run match those previously generated", {
  ...
  exp_occ_asu <- dplyr::rename(
    exp_occ_asu, `1_in_n_delay` = `X1_in_n_delay`
  )
  exp_occ_rehab <- dplyr::rename(
    exp_occ_rehab, `1_in_n_delay` = `X1_in_n_delay`
  )
